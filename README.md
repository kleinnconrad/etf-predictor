...
* **Cloud Infrastructure (GitHub Actions):** The repository includes a serverless CI/CD workflow (`.github/workflows/manual_batch.yml`) for remote execution. The workload is horizontally distributed across **20 parallel runners** to reduce wall-clock execution time. To maintain strict control over API rate limits and compute budgets, this workflow is triggered manually. Upon completion, a final merge job aggregates the distributed results and commits them directly back to the repository.
