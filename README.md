...
* **Cloud Infrastructure (GitHub Actions):** The repository includes a serverless CI/CD workflow (`.github/workflows/manual_batch.yml`) for remote execution. To maintain strict control over API rate limits and compute budgets, this workflow is triggered manually. Upon completion, the runner automatically extracts the results and commits them directly back to the repository.
