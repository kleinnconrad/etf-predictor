...
* **Economic Baseline:** The model applies an assumed base inflation rate (e.g., 2.5% p.a.) and a tolerance corridor of (e.g. -0.1% for down and +12.5% for up and flat else).
...
flowchart LR
    ...
    SignalFlat[/"<b>SIGNAL: FLAT (0)</b><br/>The market is trending sideways<br/>or the signals are unclear."/]
...
The project utilizes the Lockfile Pattern with `pip-tools` to ensure reproducible builds and conflict-free dependency resolution.

* **Top-Level Requirements:** Direct project dependencies are defined in `requirements.in`.
* **Lockfile Compilation:** The `requirements.txt` file serves strictly as an autogenerated lockfile. It is compiled from `requirements.in` using `pip-tools` and contains exact version pins for all dependencies and sub-dependencies.
* **Automated Updates:** Dependabot is configured (`.github/dependabot.yml`) to monitor the repository on a weekly schedule. When updates are available, it resolves the new dependency trees, updates `requirements.in` and `requirements.txt`, and submits a single, grouped pull request to maintain production stability.

---

## Release Process

This repository uses [Google's Release Please](https://github.com/googleapis/release-please) to automate the versioning and release process.

* **Conventional Commits:** All commits to the `main` branch must follow the [Conventional Commits](https://www.conventionalcommits.org/) specification (e.g., `feat: add new indicator`, `fix: correct threshold`).
* **Automated Pull Requests:** The `release-please` GitHub Action continuously monitors the `main` branch. When it detects new `feat` or `fix` commits, it automatically opens or updates a "Release Pull Request".
* **Changelog Generation:** This automated PR compiles all new changes into the `CHANGELOG.md` file and bumps the project version accordingly.
* **Publishing a Release:** To officially publish a new version, simply merge the open Release PR into `main`. The action will automatically tag the commit, generate a GitHub Release, and reset the cycle for the next version.
